...
export default class IncludeFlowinLWCExample extends NavigationMixin(LightningElement) {
    @api recordId;
    get inputVariables(){
        return [{
            name:"accountid",
        ...
        if(event.detail.status === 'FINISHED'){
            const flowOutputVariables = event.detail.outputVariables;
              for(let i=0; i< flowOutputVariables.length; i++){
                  ...
                  if(flowoutputvar.name === 'contactId' && flowoutputvar.value){
